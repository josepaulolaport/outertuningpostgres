...
public class CaptorPlan {
    ...
    public String getPlanExecution(String sql) {
        switch (config.getProperty("sgbd")) {
            case POSTGRESQL:
                return this.getPostgresqlPlan(sql);
            case SQLSERVER:
    ...
    public Plan getPlanExecution(String sql, Timestamp time) {
        switch (config.getProperty("sgbd")) {
            case POSTGRESQL:
                return (new PlanPostgres(this.getPlanExecution(sql), time));
            case SQLSERVER:
                return (Plan) this.notYet();
            case ORACLE:
                return (new PlanOracle(this.getPlanExecution(sql), this.getPlanExecutionIDE(sql), time));
            case MYSQL:
    ...
    private String getPostgresqlPlan(String sql) {
        String plan = config.getProperty("getPlanQueryPostgreSQL");
        plan = plan.replace("$QUERY$", sql);
        try (PreparedStatement stmt = conection.prepareStatement(plan);
             ResultSet result = stmt.executeQuery()) {
            result.next();
